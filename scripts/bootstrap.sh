...
echo "Remaining manual steps (Max \$200 Plan):"
echo ""
...
echo "     sudo -u $AGENT_USER git clone git@github.com:maruyamakoju/agent-harness.git $HARNESS_DIR"
echo ""
echo "  3. Edit .env (GITHUB_TOKEN のみ必須、API KEYは不要):"
echo "     sudo -u $AGENT_USER nano $HARNESS_DIR/.env"
echo "     # Set GITHUB_TOKEN=ghp_..."
echo "     # ANTHROPIC_API_KEY は Max Plan では不要"
echo ""
echo "  4. Build and start the agent container:"
echo "     cd $HARNESS_DIR"
echo "     sudo -u $AGENT_USER docker compose build"
echo "     sudo -u $AGENT_USER docker compose up -d"
echo ""
echo "  5. ★ Claude Code にログイン（Max Plan 認証 - 初回のみ）:"
echo "     bash $HARNESS_DIR/scripts/setup-claude-auth.sh"
echo "     # ブラウザでAnthropicアカウントにログイン"
echo "     # セッションはDocker volumeに永続化される"
echo ""
echo "  6. Apply network egress restrictions:"
...
echo ""
echo "  8. (Optional) Connect Tailscale for remote access:"
echo "     sudo tailscale up"
echo ""
echo "  9. (Optional) Setup Ollama for local LLM:"
echo "     bash $HARNESS_DIR/scripts/setup-ollama.sh"
echo ""
echo "  10. Submit your first job!:"
echo "      bash $HARNESS_DIR/scripts/create-job.sh \\"
echo "        --repo git@github.com:maruyamakoju/some-repo.git \\"
